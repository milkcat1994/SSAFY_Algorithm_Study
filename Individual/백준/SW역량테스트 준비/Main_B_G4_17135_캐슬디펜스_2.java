...
public class Main_B_G4_17135_2 {
	static int N,M;
	...
	public static int getDistance(Point e, Archer a, int time) {
		return Math.abs(e.row+time-a.row) + Math.abs(e.col-a.col);
	}
	...
	public static void inputArcher(int[] arr) {
		for(int i = 0; i<M; ++i) {
			if(arr[i] == 1)
				aSt.push(aMap.get(i));
		}
	...
	public static void pickArcher() {
		int[] arr = new int[M];
		//궁수 3명만 배치
		for(int i = M-1; i > M-1-3; --i) {
			arr[i] = 1;
		}
