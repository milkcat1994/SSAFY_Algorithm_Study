...
public class Main_B_G3_1941_소문난칠공주_2 {
	...
	public static void dfs(int index, int count, int sCount, int yCount) {
		if(yCount>3) return;
		if(count == 7) {
			if(!bfs(count)) return;
			if(sCount>=4) {
		...
		for(int i = index; i < 25; ++i) {
			...
			dfs(i+1, count+1, board[nr][nc] == 'S' ? sCount+1 : sCount, board[nr][nc] == 'Y' ? yCount+1 : yCount);
